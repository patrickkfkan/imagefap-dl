...
export function createProxyAgent(options?: ProxyOptions | null) {
  if (!options) {
    return null;
  }
  const key = createHash('md5').update(JSON.stringify(options)).digest('hex');
  if (proxyAgents[key]) {
    return proxyAgents[key];
  }
  const proxyURL = options.url;
  ...
  if (protocolMatches(protocol, [ 'http', 'https' ])) {
    const agent = createHTTPProxyAgent(options);
    proxyAgents[key] = { protocol, proxyURL, agent };
    return proxyAgents[key];
  }
  if (protocolMatches(protocol, [ 'socks4', 'socks5' ])) {
    const agent = createSocksProxyAgent(options);
    proxyAgents[key] = { protocol, proxyURL, agent };
    return proxyAgents[key];
  }
